...
def _byte(char):
    """py2 iterated bytes as 1-char str; py3 yields ints. Accept both."""
...
class Encoder:
    ...
    def encode_text_string(string):
        ...
        :rtype: list
        """
        encoded_string = [_byte(c) for c in string]
    ...
    def encode_extension_media(media_value):
        ...
        char = media_value[0]
        if _byte(char) < 32 or _byte(char) == 127:
            raise EncodeError('Invalid Extension-media: TEXT starts with '
                              'invalid character: %s' % _byte(char))
